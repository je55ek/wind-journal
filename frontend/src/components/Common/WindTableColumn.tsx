import { Box, Editable, IconButton, Portal, Select, Stack, createListCollection, useSelectContext, useEditableContext } from "@chakra-ui/react"
  ...
  return (
    <Stack direction="column" align="flex-start" w="10" gap="0">
      <Stack bg="ui.main" color="white" w="full" gap="0">
        <Box textAlign="center" >{hour_12.toString().padStart(2, "0")}</Box>
        <Box fontSize="xs" textAlign="center">{hour < 13 ? "AM" : "PM"} </Box>
      </Stack>
      <Editable.Root
        key="average"
        defaultValue=""
